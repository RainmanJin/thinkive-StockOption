...
public class MarketUtil
{
		...
		if ( tempList != null )
		{
			for (int i = 0; i < tempList.length; i++)
			{
				...
				for (int j = 0; j < typeArray.length; j++)
				{
					...
					{
						String a = stockOption.getMarket() + stockOption.getSecurityID();
	...
	*/
	...
	public static KLineData[] getWeekHQDataByDayHQ(KLineData[] dayHQ)
	{
		...
		{
			if ( !isSameWeek(dayHQ[i].getDate(), dayHQ[i + 1].getDate()) )
			{//判断是不是属于一个周内
	...
	public static KLineData[] getMonthHQDataByDayHQ(KLineData[] dayHQ)
	{
		...
		for (int i = 0; i < dayHQ.length - 1; i++)
		{
			if ( !isSameMonth(dayHQ[i].getDate(), dayHQ[i + 1].getDate()) )
			{//判断是不是属于一个月内
